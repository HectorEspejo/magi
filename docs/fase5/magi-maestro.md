...
**Última actualización:** 15 de septiembre de 2026 (Fase 5 especificada)
...
| 3 | Pestañas y Servidor de Sesiones | 79 | 3 | 4 | Implementación reportada (79/79; validación parcial de Hector con 6 arreglos incorporados; pendiente la validación completa) |
| 4 | Archivos (SFTP en panel doble) | 65 | 3 | 3,5 | Implementación reportada (65/65; 146 tests; pendiente validación manual de Hector) |
| 5 | Túneles | 54 | 3 | 3 | Especificada (sobre F2-F4 sin validar, R15) |
...
| **Total** | | **410 especificadas (~600 estimadas)** | **16** | **~19 semanas (F1-F5)** | |
...
Las tablas de F6 en adelante son previsiones y se confirman al especificar cada fase.
...
| Registro | REGISTRO | 2 | Historial de conexiones, huellas, importación/exportación, claves, sondeos, (F3) sesiones y servidor, (F4) `transferencia` y `borrado_remoto`, (F5) `tunel_abierto`, `tunel_cerrado`, `tunel_fallido`; en F6 también forzados |
...
| 5 — Túneles | 0 / 54 | ninguno (sin confirmar) |

**Progreso total acumulado:** 354 / 410 funcionalidades.
...
3. **Fase 5 — Túneles.** Especificada el 15 sep 2026 (54 funcionalidades). Ocupa `F6`; Snippets (F6 del roadmap) deberá decidir su tecla (D57: paleta/subvista o `Shift+F`).
4. **Fase 6 — Snippets y deliberación MAGI.** Snippets a etiquetas, ejecución masiva vía `Ejecutar`, deliberación con tres comprobaciones (último sondeo, backup, tests) y forzado con motivo en REGISTRO; los `reiniciar`/`deploy`/`logs` del mockup de Flota; un snippet podrá exigir un túnel activo. Sin tecla de función libre: decidir en la fase.
...
- **T36 — (F5) Las directivas de ssh_config que MAGI modela se retiran de `opciones_extra`** y pasan a la lista de gestionadas; hay una sola fuente de verdad por directiva y la exportación la devuelve al fichero.
...
| R15 | Las Fases 2, 3 y 4 tienen implementación reportada sin validación manual completa y la F5 se especifica encima (override de Hector) | Medio | Validar F2-F4 seguidas en Omarchy antes de implementar F5, o asumir arreglos mezclados |
...
| R27 | Las siete teclas de función quedan ocupadas; Snippets (F6) necesita otro acceso | Bajo | Decidir en F6: paleta + subvista, o `Shift+F1`-`F7` |
