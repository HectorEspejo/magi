...
**Última actualización:** 15 de septiembre de 2026 (Fase 4 especificada)
...
| 4 | Archivos (SFTP en panel doble) | 65 | 3 | 3,5 | Especificada (sobre F2/F3 sin validar, R15) |
...
| **Total** | | **356 especificadas (~585 estimadas)** | **13** | **~16 semanas (F1-F4)** | |
...
| Registro | REGISTRO | 2 | Historial de conexiones, huellas, importación/exportación, claves, sondeos, (F3) sesiones y servidor, (F4) `transferencia` y `borrado_remoto`; en F6 también forzados |
...
| 4 — Archivos (SFTP) | 0 / 65 | ninguno (sin confirmar) |

**Progreso total acumulado:** 289 / 356 funcionalidades.
...
2. **Fase 4 — Archivos (SFTP en panel doble).** Especificada el 15 sep 2026 (65 funcionalidades). Sin `chmod`, sin papelera, sin sincronizar directorio (candidatos posteriores).
3. **Fase 5 — Túneles.** Local, remoto y dinámico, en el servidor, con arranque automático al conectar; `LocalForward` de `opciones_extra` migrable a TUNELES; `Tuneles{lista}` y protocolo v3 (T24).
...
| R15 | Las Fases 3 y 4 se especifican sobre fases anteriores sin validación manual completa (override de Hector) | Medio | La validación parcial de F3 ya destapó 6 fallos; validar F2 y F3 antes de implementar F4, o asumir arreglos mezclados |
...
| R21 | `russh-sftp` podría no tener versión compatible con russh 0.63.3 sin parches | Medio | Fijar la última compatible al inicio del S1; si no existe, implementar el subconjunto SFTP v3 necesario sobre `Channel` (readdir, stat, open/read/write, rename, remove, mkdir) y anotarlo |
